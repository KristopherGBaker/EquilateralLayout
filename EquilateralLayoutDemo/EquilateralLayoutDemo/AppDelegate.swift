...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        let horizontalDemo = DemoViewController(numberOfItems: 80, scrollDirection: .horizontal)
        horizontalDemo.tabBarItem.image = UIImage(named: "horizontal")
        horizontalDemo.tabBarItem.title = "Horizontal"
        
        let verticalDemo = DemoViewController(numberOfItems: 80, scrollDirection: .vertical)
